            ...
            <div class="container">
                <div class="page-inner">
                    <div class="page-header">
                        <h3 class="fw-bold mb-3">Showing Item List</h3>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#formFile").change(function(event) {
                ...
                if (event.target.files.length > 0) {
                    ...
                    reader.onload = function() {
                        imagePreviewContainer.html(`<img id="imagePreview" src="${reader.result}" style="max-width: 150px; border-radius: 5px;">`);
                    };
            ...
            $(document).on("change", ".select-status", function() {
                ...
                $.ajax({
                    url: `/Sub-Category/update-status`,
                    type: "POST",
                    data: {
                        id: id,
                        status: status
        ...
        // Fetch Items & Load into DataTable
                ...
                success: function(response) {
                    if (Array.isArray(response)) {
                        ...
                        $.each(response, function(index, item) {
                            let dynamicImageUrl = `/storage/${item.item_image}`;
                            let row = [
                                counter,
                                `<div class="d-flex align-items-center">
                            <img src="${dynamicImageUrl}" alt="Item Image" width="50" height="50" class="me-2 rounded">
                            <span>${item.item_name}</span>
                        </div>`,
                                item.price,
                                item.item_type,
                                `<select class="form-select select-status" data-item-id="${item.id}">
                            <option value="1" ${item.status == 1 ? 'selected' : ''}>Active</option>
                            <option value="0" ${item.status == 0 ? 'selected' : ''}>Inactive</option>
                        </select>`,
                                `<div class="d-flex t-icon">
                            <button class="border border-0 bg-transparent editbtn me-2">
                                <a href="/Item/Update/${item.id}">
                                <i class="fas fa-edit text-primary border border-primary p-2 fs-5"></i>
                                </a>
                            </button>
                            <button class="border border-0 bg-transparent viewbtn">
                                <a href="/Item/Detail/Page/${item.id}">
                                    <i class="fas fa-eye text-success border border-success p-2 fs-5"></i>
                                </a>
                            </button>
